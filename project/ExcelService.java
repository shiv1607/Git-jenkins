...
public class ExcelService {
    ...
    public byte[] generateCollegeBookingsExcel(Long collegeId) throws IOException {
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            if (programs.isEmpty()) {
                ...
                // Create group members summary sheet
                System.out.println("👥 DEBUG: Creating group members summary sheet...");
                createGroupMembersSummarySheet(workbook, programs, headerStyle, dataStyle, titleStyle);
    ...
    private void createEmptySummarySheet(Workbook workbook, CellStyle headerStyle, CellStyle titleStyle) {
        ...
        String[] headers = {"Program Name", "Festival", "Date", "Time", "Venue", "Total Bookings", "Solo Bookings", "Group Bookings", "Total Revenue", "Total Group Members"};
        ...
        for (Program program : programs) {
            ...
            for (ProgramBooking booking : bookings) {
                if (booking.isGroupBooking()) {
                    ...
                    totalGroupMembers += groupMembers.size();
                }
            }
            ...
            // Add group members count to the summary
            row.createCell(9).setCellValue(totalGroupMembers);
    ...
    private void createGroupMembersSummarySheet(Workbook workbook, List<Program> programs, 
                                              CellStyle headerStyle, CellStyle dataStyle, CellStyle titleStyle) {
        System.out.println("👥 DEBUG: Creating group members summary sheet...");
        Sheet sheet = workbook.createSheet("Group Members Summary");
        
        // Create title
        Row titleRow = sheet.createRow(0);
        Cell titleCell = titleRow.createCell(0);
        titleCell.setCellValue("Group Members - Complete List");
        titleCell.setCellStyle(titleStyle);
        sheet.addMergedRegion(new org.apache.poi.ss.util.CellRangeAddress(0, 0, 0, 8));

        // Create headers
        Row headerRow = sheet.createRow(2);
        String[] headers = {"Program Name", "Festival", "Booking ID", "Group Leader", "Group Leader Email", "Member Name", "Member Email", "Member Phone", "Payment Status", "Amount"};
        ...
        for (Program program : programs) {
            ...
            for (ProgramBooking booking : bookings) {
                if (booking.isGroupBooking() && booking.getGroupSize() > 1) {
                    List<GroupMember> groupMembers = groupMemberRepository.findByBookingId(booking.getId());
                    ...
                    for (GroupMember member : groupMembers) {
                        ...
                        row.createCell(0).setCellValue(program.getTitle());
                        row.createCell(1).setCellValue(program.getFest().getTitle());
                        row.createCell(2).setCellValue(booking.getId());
                        row.createCell(3).setCellValue(booking.getStudentName());
                        row.createCell(4).setCellValue(booking.getStudentEmail());
                        row.createCell(5).setCellValue(member.getMemberName());
                        row.createCell(6).setCellValue(member.getMemberEmail());
                        row.createCell(7).setCellValue(member.getMemberPhone());
                        row.createCell(8).setCellValue(booking.getPaymentStatus().toString());
        ...
        System.out.println("✅ DEBUG: Group members summary sheet created successfully");
    }
    ...
    private void createProgramSheet(Workbook workbook, Program program, 
        ...
        String[] headers = {"Student Name", "Email", "Phone", "Booking Type", "Group Size", "Payment Status", "Transaction ID", "Amount", "Group Members", "Member Details"};
        ...
        for (ProgramBooking booking : bookings) {
            ...
            if (booking.isGroupBooking() && booking.getGroupSize() > 1) {
                List<GroupMember> groupMembers = groupMemberRepository.findByBookingId(booking.getId());
                row.createCell(8).setCellValue(groupMembers.size() + " members");
                
                // Create member details string
                StringBuilder memberDetails = new StringBuilder();
                for (int i = 0; i < groupMembers.size(); i++) {
                    GroupMember member = groupMembers.get(i);
                    memberDetails.append(i + 1).append(". ").append(member.getMemberName())
                               .append(" (").append(member.getMemberEmail()).append(")");
                    if (i < groupMembers.size() - 1) {
                        memberDetails.append("\n");
                    }
                }
                row.createCell(9).setCellValue(memberDetails.toString());
            } else {
